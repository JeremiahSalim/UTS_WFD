...
<form action="{{ route('edit.store', $data->id) }}" method="POST" class="space-y-4">
    @csrf
    @method('PUT')
    <div>
        <label class="block text-sm font-medium text-gray-700">Name</label>
        <input type="text" name="nama_pemesan" required class="w-full px-4 py-2 border rounded-md shadow-sm" value="{{ $data->nama_pemesan }}">
    </div>
    
    <div>
        <label class="block text-sm font-medium text-gray-700">Whatsapp</label>
        <input type="tel" name="wa_pemesan" required class="w-full px-4 py-2 border rounded-md shadow-sm" value="{{ $data->wa_pemesan}}">
    </div>

    <div>
        <label class="block text-sm font-medium text-gray-700">Tanggal Booking</label>
        <input type="date" name="tanggal" required class="w-full px-4 py-2 border rounded-md shadow-sm" value="{{ $data->tanggal }}">
    </div>
    ...
    <div>
        ...
        <select name="nomor_lapangan" id="nomor_lapangan" class="bg-transparent w-full px-4 py-2 border rounded-md shadow-sm">
            <option class="" value="{{ $data->jadwal->nomor_lapangan }}" selected hidden>{{ $data->jadwal->nomor_lapangan }}</option>
    ...
    <div>
        ...
        <select name="jam_pemakaian" id="jam_pemakaian" class="bg-transparent w-full px-4 py-2 border rounded-md shadow-sm">
            <option class="" value="{{ $data->jadwal->jam_mulai }}" selected hidden>{{ $data->jadwal->jam_mulai }} - {{ $data->jadwal->jam_selesai }}</option>
